...
지금 상태: **데모 3종 시나리오가 x402 왕복으로 처음부터 끝까지 돌아간다.** 실제 온체인 USDC 결제가 발생하고, 대시보드에 실시간으로 찍히고, 에이전트가 402 챌린지를 받아 조건을 선택해 정산까지 확정한다. 남은 건 신용점수 실계산, 거부·예약 시나리오, P2P 직거래(Phase 2.5), 배포, 그리고 제출물(영상·PPT).
...
### Claude가 이어서 — WBS Phase 2

**Phase 1(x402 에이전트 연결)은 7/27 회사 맥에서 완료.** 가맹점 그래프에 `request_terms`
노드가 들어가 402 챌린지 → 조건 선택 → 결제 → 서명 제출 → 정산 확정이 한 왕복으로 돈다.
본사의 사후 검증은 x402 영수증이 대체한다 (영수증이 없을 때만 `payment.verify` 폴백).
→ 다음은 [wbs.md](wbs.md) Phase 2(신용점수 실계산·거부 시나리오·예약 실행)

주의: 데모를 여러 번 돌리면 가맹점 USDC가 소진돼 **A·B지점이 결제 대신 유예를 제안한다**
(코드 문제가 아니다). §5의 보충 명령으로 store-a·b만 채우면 된다.
