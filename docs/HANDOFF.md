...
**산출물** (`~/Desktop/solply-video/`, **2분 55초** — 한도 3분, 1600×900):
...
구성(2:55): 오프닝(쌓인 라이브 기록) → 정상 결제 + Explorer 실제 tx →
...
`docs/pitch.html` **17장**. 팀명 **Solply**, 라이브 URL 반영.

**8/1 디자인 재작업** — 사용자 지적("너무 심플하고 AI가 만든 것 같다, 시각화가 없다"):
- **실제 제품 화면 5장을 브라우저 창 프레임에 넣었다** (`docs/shots/*.jpg`, 604KB).
  라이브에서 목적에 맞게 새로 찍은 것 — `scripts/video/shots.mjs`로 재촬영 가능.
- 레이아웃을 다양화했다: 좌 텍스트·우 화면 분할(`.split`), 번호 짚어주기(`.point`),
  큰 숫자(`.stat`), 가로 흐름 도식(`.flow`), 강조 띠(`.band`). 이전엔 거의 전부 `.rows`였다.
- **화면이 슬라이드의 주장을 증명하게** 맞췄다 — pay.sh 슬라이드의 로그 캡처에 정작
  `시세 데이터 구매` 줄이 없어서 다시 찍었다. 캡처를 바꾸면 이 대응을 꼭 확인할 것.
- **덤으로 잡은 것**: 협상 슬라이드 숫자가 라이브와 달랐다(2.5·32.5·42.5 USDC → 실제는
  7.00 → 6.50, 차감 0.5). 금액을 1/5로 축소한 7/28 이후 소개서만 옛 숫자로 남아 있었다.
추가한 두 장이 7/28 이후 만든 것을 메운다 — **"발표용 화면이 아닙니다. 지금도 거래하고
있습니다"**(라이브 누적 실적·`/shop` 참여·날짜 이동)와 **"결제 레일 셋이 서로 다른 일을
합니다"**(USDC·x402·pay.sh 역할 구분 + `pay server` 다음 단계).

**PDF**: `~/Desktop/solply-video/Solply-소개서.pdf` — **17쪽 벡터**(13.333×7.5in, 16:9).
재생성은 `node scripts/video/deck.mjs` (넘침 검사까지 해준다).
다시 뽑으려면 `node scripts/video/deck.mjs`가 아니라 Chrome에서 `docs/pitch.html`을 열고
인쇄 → PDF로 저장(배경 그래픽 켜기). **영상 URL만 업로드 후 마지막 장에 채우면 된다.**

### ✅ 4. README 정비 (8/1 완료 — 심사자 첫 화면 기준으로 재작성)
