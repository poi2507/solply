...
export function metricsFor(role, scoped, wallets) {
  ...
  if (role.kind === "store") {
    ...
    return [
      ...
      { label: "내 신용점수", value: me.creditScore ?? 0, unit: "점", foot: me.creditBasis ?? "납부 이력 기준", plain: true },
      { label: "지갑 잔액", value: wallet?.usdc ?? 0, unit: "USDC", foot: "결제 가능액", accent: true },
    ];
  }
  ...
  if (role.kind === "admin") {
    return [
      ...
      { label: "사람 개입", value: t.humanActions, unit: "회", foot: "버튼을 누른 횟수", accent: true },
