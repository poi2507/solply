...
from app.agents.store import prompt
from app.db import store as db
from app.solana import payments


def make_tools(store_id: str, spend_limit: float) -> list:
    """지점에 바인딩된 도구 묶음을 만든다."""
    ...
    def verify_delivery(invoice_id: str) -> dict:
        ...
        utils.log(actor, "delivery.verified", result)
        return result
    ...
    def execute_payment(invoice_id: str) -> dict:
        ...
        if invoice["amount_usdc"] > spend_limit:
            utils.log(actor, "payment.blocked_over_limit", {"invoice_id": invoice_id})
            return {
    ...
    def propose_adjustment(invoice_id: str, deduction_usdc: float, reason: str) -> dict:
        ...
        utils.log(actor, "proposal.adjustment", proposal)
        return proposal
    ...
    def propose_deferral(invoice_id: str, pay_when: str, reason: str) -> dict:
        ...
        utils.log(actor, "proposal.deferral", proposal)
        return proposal
    ...
    def refuse_payment(invoice_id: str, reason: str) -> dict:
        ...
        utils.log(actor, "payment.refused", {"invoice_id": invoice_id, "reason": reason})
...
def build(store_id: str | None = None, spend_limit: float | None = None):
    ...
    return Agent(
        ...
        instruction=prompt.system(store_id, spend_limit),
        tools=tools,
    )
