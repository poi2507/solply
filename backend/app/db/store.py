...
def sum_by(collection: str, group_key: str, value_key: str, **filters) -> dict[str, float]:
    """그룹별 합계 — 재고처럼 이력을 접어야 하는 조회용."""
    return _store.sum_by(collection, group_key, value_key, **filters)
...
def recent_events(limit: int, day: str | None = None,
                  action: str | None = None) -> list[dict]:
    """최근 N건만 (최신순). day를 주면 그 하루 안에서."""
    return _store.recent_events(limit, day, action)
